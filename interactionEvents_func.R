...
contact_events<-function(data,xy,id,outfile,spdist,tdist,pairs){
  dat<-data
  #dat$time<-as.numeric(dat$datetime)
  colnames(dat)[colnames(dat)==xy[1]]<-'x'
  colnames(dat)[colnames(dat)==xy[2]]<-'y'
  colnames(dat)[colnames(dat)==id]<-'host'
...
contact_events(data = dat,c('x','y'),'host',2,5*60*24,'D:/test/',pairs)
